import { genSnake, genTarget, isCrash, isEat, changeSnake } from ".\\data.js";
...
Game.prototype.start = function(){
    ...
    this.snake = genSnake(this.width,this.height,this.initS,this.rectR);
    this.target = genTarget(this.width,this.height,this.rectR,this.snake);
    //fps 60 paint
...
Game.prototype.run = function(){
    this.runTask = setInterval(()=>{
        //snake run
        if(isCrash(this.width,this.height,this.snake)){
            this.over();
        }
        else{
            if(isEat(this.snake,this.target)){
                this.score += 100;
                this.refreshScore();
                //eat
