import {
    genSnake as data_genSnake,
    genTarget as data_genTarget,
    isCrash as data_isCrash,
    isEat as data_isEat,
    changeSnake as data_changeSnake,
} from ".\\data.js";

import { paint as canvas_paint } from ".\\canvas.js";
var index;
...
Game.prototype.start = function(){
    ...
    this.snake = data_genSnake(this.width,this.height,this.initS,this.rectR);
    this.target = data_genTarget(this.width,this.height,this.rectR,this.snake);
    //fps 60 paint
...
Game.prototype.run = function(){
    this.runTask = setInterval(()=>{
        //snake run
        if(data_isCrash(this.width,this.height,this.snake)){
            this.over();
        }
        else{
            if(data_isEat(this.snake,this.target)){
                this.score += 100;
                this.refreshScore();
                //eat
